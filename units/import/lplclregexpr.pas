...
uses
  Classes, SysUtils, RegExpr, lpcompiler, lptypes, ulpClassHelper;
...
//constructor Create;
procedure TRegExpr_Init(const Params: PParamArray; const Result: Pointer);cdecl;
begin
  PRegExpr(Params^[0])^ := TRegExpr.Create(PLPString(Params^[1])^);
end;

//function Exec (const AInputString : RegExprString) : boolean; {$IFDEF OverMeth} overload;
procedure TRegExpr_Exec(const Params: PParamArray; const Result: Pointer);cdecl;
begin
  PBoolean(Result)^ := PRegExpr(Params^[0])^.Exec(PLPString(Params^[1])^);
end;

//function ExecNext : boolean;
procedure TRegExpr_ExecNext(const Params: PParamArray; const Result: Pointer);cdecl;
begin
  PBoolean(Result)^ := PRegExpr(Params^[0])^.ExecNext();
end;

//function ExecPos: boolean;
procedure TRegExpr_ExecPos(const Params: PParamArray; const Result: Pointer);cdecl;
begin
...
procedure TRegExpr_InputString_Read(const Params: PParamArray; const Result: Pointer);cdecl;
begin
...
begin
  ...
  begin
    ...
    addGlobalFunc('procedure TRegExpr.Init(s: string);', @TRegExpr_Init);
